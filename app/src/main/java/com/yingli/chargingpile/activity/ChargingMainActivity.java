...
import com.yingli.chargingpile.EventBusMsg.AddDevMsg;
import com.yingli.chargingpile.EventBusMsg.FreshListMsg;
import com.yingli.chargingpile.EventBusMsg.FreshTimingMsg;
import com.yingli.chargingpile.EventBusMsg.RefreshRateMsg;
import com.yingli.chargingpile.EventBusMsg.SearchDevMsg;
import com.yingli.chargingpile.R;
import com.yingli.chargingpile.adapter.ChargingListAdapter;
import com.yingli.chargingpile.adapter.GunSwitchAdapter;
import com.yingli.chargingpile.adapter.ReservaCharingAdapter;
import com.yingli.chargingpile.application.MyApplication;
import com.yingli.chargingpile.bean.ChargeStartFreshMsg;
...
import com.yingli.chargingpile.bean.PileSetBean;
import com.yingli.chargingpile.bean.ReservationBean;
import com.yingli.chargingpile.connutil.PostUtil;
import com.yingli.chargingpile.util.CircleDialogUtils;
...
import com.yingli.chargingpile.view.RoundProgressBar;
...
public class ChargingMainActivity extends BaseActivity implements BaseQuickAdapter.OnItemClickListener {
    ...
    ConstraintLayout rlChargingSet;
    ...
    private void initPreparingView() {
        ...
        rlPpmoneyEdit.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(ChargingMainActivity.this, ChargingPresetEditActivity.class);
        ...
        ivResever.setOnClickListener(v -> {
            ...
            if (presetType == 3) {
                ...
                if (TextUtils.isEmpty(duration) || duration.contains("-")) {//未选择时长
                    //去预约列表操作
                    Intent intent = new Intent(ChargingMainActivity.this, ChargingDurationActivity.class);
...
//                startChargeAnim();
    ...
    private void setPowerLimit() {
        //弹出时停止刷新
        mFreshTimer.timerPause();
        View view = LayoutInflater.from(ChargingMainActivity.this).inflate(R.layout.popuwindow_power_limit, null);
...
//        if (solarMode > 2 || solarMode < 0) solarMode = 0;
    ...
    private void addChargingPile() {
        if (SmartHomeUtil.isFlagUser()) {//浏览账户
            FragmentManager fragmentManager = ChargingMainActivity.this.getSupportFragmentManager();
            new CircleDialog.Builder()
...
//                            params.backgroundColor=ContextCompat.getColor(ChargingPileActivity.this, R.color.preset_edit_time_background);
                        params.strokeColor = ContextCompat.getColor(ChargingMainActivity.this, R.color.preset_edit_time_background);
                    })
                            ...
                            PostUtil.postJson(SmartHomeUrlUtil.postGetDemoCode(), json, new PostUtil.postListener() {
                                ...
                                public void success(String json) {
                                    try {
                                        ...
                                        int code = object.getInt("code");
                                        if (code == 0) {
                                            Intent intent = new Intent(ChargingMainActivity.this, AddChargingActivity.class);
                                            jumpTo(intent, false);
                                        } else {
                                            String data = object.getString("data");
                                            toast(data);
    ...
    private void requestDelete(final ChargingBean.DataBean bean) {
        ...
        new CircleDialog.Builder()
            ...
            LogUtil.d("删除充电桩");
            Mydialog.Show(ChargingMainActivity.this);
            Map<String, Object> jsonMap = new HashMap<>();
