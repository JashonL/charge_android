package com.yingli.chargingpile.scan;
...
import com.yingli.chargingpile.scan.activity.MyCaptureActivity;
...
public class MyCaptureActivityHandler extends Handler{
    ...
    private final MyCameraManger cameraManager;

    private enum State {
        PREVIEW, SUCCESS, DONE
    }

    public MyCaptureActivityHandler(MyCaptureActivity activity,MyCameraManger cameraManager) {
        this.activity = activity;
        decodeThread = new MyDecodeThread(activity,  new MyViewfinderResultPointCallback(activity.getViewfinderView()));
        decodeThread.start();
        state = MyCaptureActivityHandler.State.SUCCESS;
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            ...
            case Constant.DECODE_FAILED:
                ...
                state = MyCaptureActivityHandler.State.PREVIEW;
    ...
    public void quitSynchronously() {
        state = MyCaptureActivityHandler.State.DONE;
    ...
    public void restartPreviewAndDecode() {
        if (state == MyCaptureActivityHandler.State.SUCCESS) {
            state = MyCaptureActivityHandler.State.PREVIEW;
            cameraManager.requestPreviewFrame(decodeThread.getHandler(),
                    Constant.DECODE);
